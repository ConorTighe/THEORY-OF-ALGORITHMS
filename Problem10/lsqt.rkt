...
(define (lstq l m) ; define operation function
  ...
  (if (null? l) ; check if l is null
      ...
      (if (null? m) ;check if m is null
          ...
          (sumList (list (sqre (- (car l)(car m))) (lstq (cdr l)(cdr m))))))) ; take m from l, square the result, put square result into list, pass to sumList
...
(display y)
(display "\nResults:\n")
(lstq x y) ; Pass lists x and y to function
...
(display "\nResults:\n")
(lstq testx testy) ; Pass Test case lists to function